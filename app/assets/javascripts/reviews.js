$(document).on('ajax:success', 'form#comments-form', function(ev, data) {
	...
	$("div.comments").append("<div class='comment'><div class='comment-header pull-left'><img class='comment-avatar' src='http://placehold.it/60x60' alt='Avatar'><div class='comment-data pull-left'><h4 class='comment-user'>" + data.user.username + "<span>" + data.review.created_at + "</span></h4><h4 class='comment-title'>" + data.review.title + "<i class='pull-right stars rate-"+ data.review.score + "'></i></h4></div></div><div class='comment-body pull-left'><p>" + data.review.body + "</p></div></div>");
	$("#review_title").val("");
	$("#review_body").val("");
